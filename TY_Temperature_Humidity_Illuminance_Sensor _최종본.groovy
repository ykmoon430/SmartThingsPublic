...
def parse(String description) {
	...
	if (!map) {
		...
		map.descriptionText = temperatureScale == 'C' ? "$device.displayName was $map.value¡ÆC" : "$device.displayName was $map.value¡ÆF"
	...
	def results = map ? sendEvent(map) : [:]
 	setStatus(device.currentValue("temperature")+"¡ÆC/"+device.currentValue("humidity")+"%/"+device.currentValue("illuminance")+"lux")
	return results;
}
